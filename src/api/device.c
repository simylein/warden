...
uint16_t device_select_one(sqlite3 *database, bwt_t *bwt, device_t *device, response_t *response) {
	...
	if (result == SQLITE_ROW) {
		...
		const uint64_t created_at = (uint64_t)sqlite3_column_int64(stmt, 5);
		const uint64_t updated_at = (uint64_t)sqlite3_column_int64(stmt, 6);
		...
		const uint64_t uplink_received_at = (uint64_t)sqlite3_column_int64(stmt, 8);
		...
		const uint64_t reading_captured_at = (uint64_t)sqlite3_column_int64(stmt, 12);
		...
		const uint64_t metric_captured_at = (uint64_t)sqlite3_column_int64(stmt, 16);
